...
key("n", "<leader>ff", function() fzf.files() end, { desc = "Find files" })
key("n", "<leader>fg", function() fzf.live_grep() end, { desc = "Grep files" })
...
-- === group: (s)earch ===
key("n", "<leader>st", "<cmd>TodoFzfLua<cr>", { desc = "Find all todo's" })
key("n", "<leader>sT", "<cmd>TodoFzfLua keywords=TODO,FIX,FIXME<cr>", { desc = "Find only Todo/Fix/Fixme" })
...
-- === group: (g)o ===
key("n", "<leader>gd", function() fzf.lsp_definitions() end, { desc = "Go to definition" })
key("n", "<leader>gr", function() fzf.lsp_references() end, { desc = "Go to reference" })

-- === group: (c)ode ===
key("n", "<leader>cs", function() fzf.lsp_document_symbols() end, { desc = "File symbols (functions, classes, etc)" })
key({ "n", "v" }, "<leader>cf", function()
    require("conform").format({ async = true, lsp_fallback = true })
end, { desc = "Code format (buffer)" })

-- === group: (b)uffers ===
key("n", "<leader>bb", function() fzf.buffers() end, { desc = "Buffer switch" })
...
-- === group: (o)verseer ===
key("n", "<leader>oo", "<cmd>OverseerToggle<cr>", { desc = "Overseer: toggle panel" })
key("n", "<leader>or", "<cmd>OverseerRun<cr>", { desc = "Overseer: run task" })
key("n", "<leader>ol", "<cmd>OverseerLoadBundle<cr>", { desc = "Overseer: load bundle" })
...
-- === group: (h)elp ===
key("n", "<leader>hn", function() fzf.help_tags() end, { desc = "Search nvim docs" })
...
-- === Aliases ===
key("n", "<leader>,", function() fzf.buffers() end, { desc = "Buffer switch" })
key("n", "<leader>.", function() fzf.files({ hidden = true }) end, { desc = "Find/Create file" })
key("n", "<leader><space>", function() fzf.files() end, { desc = "Find Files" })
key("n", "<leader>/", function() fzf.live_grep() end, { desc = "Grep files" })

key({ "n", "v" }, "-", "<cmd>Oil<cr>", { desc = "Open Oil", silent = true })

-- === Diagnostics ===
key("n", "<leader>cd", vim.diagnostic.open_float, { desc = "Show diagnostic" })
